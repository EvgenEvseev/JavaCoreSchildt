...
public class FileInputStreamDemo {
    public static void main(String[] args) {
        int size;
        try(FileInputStream f= new FileInputStream("C:\\Users\\Компрютер1\\IdeaProjects\\JavaCoreSchildt\\src\\Chapter20\\FileInputStreamDemo.java")){
            ...
            byte b[]=new byte[n];
            if(f.read(b)!=n){
                System.err.println("Нельзя прочитатать "+n+" байтов");
            }
            System.out.println(new String(b,0,n));
            System.out.println("\nВсе еще доступно "+(size=f.available()));
            System.out.println("Пропустить оставшуюся часть методом skip()");
            f.skip(size/2);
